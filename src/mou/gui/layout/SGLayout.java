/*
 * @(#)SGLayout.java 1.02 2 Jan 2002 Copyright John Redmond (John.Redmond@mq.edu.au). This software
 * is freely available for commercial and non-commercial purposes. Acknowledgement of its source
		...
		implements LayoutManager, java.io.Serializable
{
	...
	protected int _hAlignment = FILL, _vAlignment = FILL;
	protected int[][] hAlignments, vAlignments;
	protected int[] rowSizes, columnSizes;

	/**
	 * Creates a default (2 x 2) layout with the specified number of rows and columns.
	 * <p>
	 * horizontal and vertical gaps are set to 0 and X- and Y-alignments are set to FILL.
	 */
	public SGLayout()
	{
	...
	/**
	 * Creates a layout with the specified number of rows and columns and specified gaps and
	 * alignments.
	 * <p>
	 * horizontal and vertical gaps are set to 0 and X- and Y-alignments are set to FILL.
	 * 
	...
	private void setAlignments()
	{
		...
		for(int i = 0; i < rows; i++)
		{
			for(int j = 0; j < cols; j++)
			{
				hAlignments[i][j] = _hAlignment;
				vAlignments[i][j] = _vAlignment;
			}
		}
	}
	...
	/*
	 * public void setAlignment(int h, int v) { hAlignment = h; vAlignment = v; setAlignments(); }
	 */
	/**
	 ...
	 * <p>
	 * The preferred width is the width of the largest row of children, which is the largest sum of
	 * preferred widths.
	 * <p>
	 * The preferred height is the sum of the the largest heights of the rows, which is the largest
	 * preferred height in each row.
	 * 
	 * @param target
	...
	/**
	 * Lay out the specified container using this layout within the calculated grids.
	 * <p>
	 * 
	 * @param parent
	...
	{
		/*
		 * int maxWidth = parent.getSize().width - leftBorder - rightBorder; int maxHeight =
		 * parent.getSize().height - topBorder - bottomBorder; int previousWidth = 0, previousHeight =
		 * 0; int rowh = 0, start = 0; Dimension d = new Dimension(60, 30);
		 */
	...
	/**
	 * Set the bounds for a component of specified coordinates. given the cell coordinates and the
	 * origin of the cell.
	 * <p>
	 * 
	 * @param row
	...
	void setComponentBounds(Component c, int row, int column, int left, int top)
	{
		...
		if(xSpace > 0)
		{
			...
			if(alignment != FILL) finalWidth = d.width; // reduce width to
			// preferred val
		}
		...
		if(ySpace > 0)
		{
			...
			if(vAlignment != FILL) finalHeight = d.height; // reduce height to
			// pref val
		}
	...
	/**
	 * Update out the maximum sizes for each of the grid cells using the specified scale values for
	 * rows and columns.
	 * 
	 * @param parent
	 *            the container to be laid out.
