/*
 * $Id: IntrospectorTreeNode.java 12 2006-03-25 16:19:37Z root $ Created on Mar 25, 2006 Copyright
...
public class IntrospectorTreeNode extends DefaultMutableTreeNode
{
	...
	{
		super.remove(aChild);
		// Jetzt von unterliegende Map Daten entfernen
		if(map != null)
			map.remove(((IntrospectorTreeNode) aChild).getEntryKey());
		else if(collection != null) collection.remove(((IntrospectorTreeNode) aChild).getUserObject());
	}
	...
	public void remove(int childIndex)
	{
		...
		if(map != null)
			map.remove(((IntrospectorTreeNode) child).getEntryKey());
		else if(collection != null) collection.remove(((IntrospectorTreeNode) child).getUserObject());
	}
	...
	/**
	 * Liefert Value aus der unterliegenden Map, mit dem diese Node verknüft ist
	 * 
	 * @return
	 */
